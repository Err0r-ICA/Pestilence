			...
			try:
				if self.currentmodule is not None:
					try:
						...
						cmd = raw_input(
							bold(green('\nroot@')) + bold(blue(just_print)) + green('pestilence -->>> ')).strip()
					except:
						self.currentmodule = None
	...
	def actOnCommand(self, cmd):
		try:
			...
			if cmd == 'report-mal':
				...
				email += "\x40"
				email += "morirt\x2ecom"
				print("-------------- Begin of Pestilence Report --------------")
				print(report)
				print("-------------- Ending of Pestilence Report --------------")
			...
			if cmd == 'list all':
				print("\nAvailable Payloads:\033[38;5;203m")
				manySearch = manysearches.MuchSearch()
